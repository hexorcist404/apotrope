    ...
    if not volumes:
        return [CheckResult(
            ...
            command=(
                ...
                "    # way back in once the TPM stops releasing the key -- a firmware\n"
                "    # update, a board swap, a boot-order change. The cmdlets above\n"
                "    # return a volume object whose default table shows the protector\n"
                "    # TYPES but not the password, and it cannot be read back from the\n"
                "    # recovery prompt itself.\n"
...
def _check_volume(vol: dict) -> list[CheckResult]:
    ...
    return [CheckResult(
        ...
        command=(
            ...
            "    $rp = (Get-BitLockerVolume -MountPoint '" + mount + "').KeyProtector | "
            "Where-Object KeyProtectorType -eq 'RecoveryPassword'\n"
            "    $rp | Format-List KeyProtectorId, RecoveryPassword\n"
            "    # Domain- or Entra-joined? Escrow it centrally as well:\n"
            "    # Backup-BitLockerKeyProtector -MountPoint '" + mount + "' "
            "-KeyProtectorId $rp.KeyProtectorId\n"
            "    # BackupToAAD-BitLockerKeyProtector -MountPoint '" + mount + "' "
            "-KeyProtectorId $rp.KeyProtectorId\n"
            "} else {\n"
            "    Write-Warning 'BitLocker is unavailable on this Windows edition.'\n"
            "}"
