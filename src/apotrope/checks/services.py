...
def _check_unquoted_paths() -> list[CheckResult]:
    ...
    return [CheckResult(
        ...
        command=(
            ...
            "# One anchored capture yields both halves: the data, and the TYPE\n"
            "# COLUMN that decides the kind. The kind must come from that column\n"
            "# alone — searching the whole line reads the path data too, so a\n"
            "# REG_SZ whose path merely CONTAINS 'REG_EXPAND_SZ' would be written\n"
            "# back as REG_EXPAND_SZ, turning on %variable% expansion for a\n"
            "# boot-start service: a semantics change nobody asked for.\n"
            "if ($line -notmatch "
            "'^\\s*ImagePath\\s+(?<type>REG_SZ|REG_EXPAND_SZ)\\s+(?<data>.*)$') {\n"
            "    throw \"unrecognized ImagePath value line for $svc\"\n"
            "}\n"
            "# Read both out of $matches now — the exe/args match below replaces it.\n"
            "$img = $matches['data']\n"
            "$kind = if ($matches['type'] -eq 'REG_EXPAND_SZ') "
